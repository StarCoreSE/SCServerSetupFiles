...
FOR /D %%D IN (torch-server-*) DO (
    ...
    IF EXIST "%%D\Torch.cfg" (
        ...
        powershell -Command "(Get-Content '%%D\Torch.cfg') -replace '<Autostart>true</Autostart>', '<Autostart>false</Autostart>' | Set-Content '%%TEMP_FILE%'"
        MOVE /Y "%%TEMP_FILE%" "%%D\Torch.cfg"

        REM Replace <NoGui>false</NoGui> with <NoGui>true</NoGui>
        powershell -Command "(Get-Content '%%D\Torch.cfg') -replace '<NoGui>true</NoGui>', '<NoGui>false</NoGui>' | Set-Content '%%TEMP_FILE%'"
        MOVE /Y "%%TEMP_FILE%" "%%D\Torch.cfg"
